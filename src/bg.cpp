...
const int Background::render() const
{
    ...
    SDL_Rect src;
    SDL_Rect dst;
    src = SDL_Rect{SCREEN_WIDTH, 0, SCREEN_WIDTH, int((app->tile_scroll * TILE_SIZE + app->scroll * TILE_SIZE) * BACKGROUND_SCROLL) % SCREEN_HEIGHT};
    ...
    code |= image.render(&src, &dst);
    src = SDL_Rect{0, 0, SCREEN_WIDTH, int((app->tile_scroll * TILE_SIZE + app->scroll * TILE_SIZE) * FOREGROUND_SCROLL) % SCREEN_HEIGHT};
    dst = SDL_Rect{0, SCREEN_HEIGHT - src.h, SCREEN_WIDTH, src.h};
    code |= image.render(&src, &dst);
